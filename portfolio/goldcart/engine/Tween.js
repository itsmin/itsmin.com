...
function Tween(_args) {
	...
	if (!this.delay) this.start = Game.logicClock;
}
...
Tween.prototype.logic = function() {
	...
	if (this.delay > 0) {
		this.delay--;
		if (this.delay <= 0) this.start = Game.logicClock;
		else return;
	}
	
	// calculate current progress
	this.amount = (Game.logicClock - this.start) / this.duration;
	if (this.amount >= 1.0) {
		this.amount = 1.0;
		this.destroyed = true;
	}
	...
	if (this.onTweenUpdate) this.onTweenUpdate();
